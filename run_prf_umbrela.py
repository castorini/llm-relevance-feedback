...
MAX_TOP_DOCS_ALLOWED = 8

def get_relevant_docids(pseudo_qrels, qid):
    max_rel_judgement_scores =  max([int(pseudo_qrels[qid][docid]['relevance_label']) for docid in pseudo_qrels[qid].keys() \
                                if pseudo_qrels[qid][docid]['relevance_label'] is not None])

    if max_rel_judgement_scores != 0:
        relevant_docids = [
            (docid, pseudo_qrels[qid][docid]['bm25_score'])
            for docid in pseudo_qrels[qid]
            if pseudo_qrels[qid][docid]['relevance_label'] is not None
            and int(pseudo_qrels[qid][docid]['relevance_label']) == max_rel_judgement_scores]
    else:
       relevant_docids = []
    return relevant_docids

def run_llm_relevance_feedback(llm_judge, bm25_outputs, hits_judged, dataset):
    qids_topk, queries_topk, docids_topk, passages_topk, bm25_scores_new = \
        topk_flattened(bm25_outputs, k=hits_judged)

    import time
    start = time.perf_counter()
    relevance_labels = llm_judge.predict(queries=queries_topk, 
                                         passages=passages_topk,
                                         gen_task='relevance_assessment',
                                         dataset=dataset,
                                         )
    end = time.perf_counter()
    total_time = end - start
    num_queries = len(set(queries_topk))
    avg_time_per_query = total_time / num_queries
    print(f"Total time: {total_time:.4f}s")
    print(f"LLM-JUDGE JUDGING {hits_judged} documents, dataset={dataset}")
    print(f"Avg Time Per Query: {avg_time_per_query:.4f}s", f"Num Queries: {num_queries}")
    pseudo_qrels = {} 
    for qid, docid, relevance, scores in zip(qids_topk,
                                             docids_topk, 
                                             relevance_labels,
                                             bm25_scores_new):
        if qid in pseudo_qrels:
            pseudo_qrels[qid][docid] = {'relevance_label': relevance, 
                                        'bm25_score': scores}
        else:
            pseudo_qrels[qid] = {docid: {'relevance_label': relevance, 
                                          'bm25_score': scores}}
...
def generate_rm3_query(qid, 
                       ...
                       hyde_vectors=None, 
                       hyde_scores=None):
    # First, we fetch the max relevance score from our pseudo relevance labels
    ...
    if hyde_vectors is not None:
        assert(hyde_scores is not None)
        rel_feedback_vectors = rel_feedback_vectors + hyde_vectors
        scores = scores + hyde_scores
...
def generate_avg_dense_prf(qid, 
                           query, 
                           pseudo_qrels, 
                           faiss_searcher, 
                           query_encoder, 
    ...
    feedback_vectors = [faiss_searcher.index.reconstruct(docid_dict[doc_id]) \
                            for doc_id, _ in relevant_docids] 
    if hyde_vectors is not None:
        feedback_vectors = feedback_vectors + hyde_vectors
    ...
    all_emb = all_emb.reshape((1, len(all_emb)))
    return all_emb

def generate_rocchio_dense_prf(qid, 
                               query, 
                               pseudo_qrels, 
                               faiss_searcher, 
                               query_encoder, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hits_judged', type=int, default=10, help='search results that the LLM judges')
    ...
    parser.add_argument('--use_hyde_vectors', action='store_true') 
    ...
    if args.hyde_precomputed_passages is not None:
        ...
        if args.use_hyde_vectors:
            hyde_vectors = list(hyde_vectors)
        else:
            print("NOT USING HYDE VECTORS!")
            hyde_vectors=[None]*len(queries)
    else:
    ...
    if args.hyde_precomputed_passages is not None:
        filename = f'{filename}_using-w-hyde_vectors-{args.use_hyde_vectors}'
    ...
    if args.final_encoder == 'bm25':
        if 'rm3' in args.feedback_mechanism:
            if args.hyde_precomputed_passages is not None:
                hyde_score = [
                    [compute_bm25_score(query, passage, index_reader_final) for passage in passages]
                    for query, passages in zip(queries, synthetic_passages)
                ]
            else:
                hyde_score = [None]*len(queries)
            ...
            raise ValueError(f"Unsupported sparse feedback mechanism: {args.feedback_mechanism}")
    else:
